# MobileBiHap
Real-time bi-manual haptic interaction system using smartphones and Unity3D. Turns everyday smartphones into intuitive, low-cost haptic controllers.

<img width="1281" height="652" alt="fig  3" src="https://github.com/user-attachments/assets/61ec5100-5818-4beb-847d-c61cf22b81a7" />

## Overview
MobileBiHap is a smartphone-based real-time bi-manual haptic interaction system. Instead of relying on costly and cumbersome haptic devices, MobileBiHap uses everyday smartphones to control skinned 3D hand models in Unity3D. Pinch, drag, and rotation gestures are transmitted via UDP, enabling translation, rotation, and grasp/release interactions. Bidirectional haptic feedback—smartphone vibration and sound—provides immersive touch cues. With two smartphones, users can independently control left and right hands for coordinated bimanual interaction. This system explicitly focuses on bi-manual interaction, enabling independent yet coordinated control of two virtual hands using separate smartphones.
...
Detect multi-touch gestures (pinch, drag, rotation). Convert them into control commands with selected hand (Left/Right) and axis mode (Y or Z). When the touch ends, the state returns to Release. Smartphones also receive haptic commands to trigger vibration and audio.
...
Bake the skinned hand mesh and use V-HACD convex colliders for robust physics contact. On Grab, objects follow the hand; on Release, they detach and fall naturally. Collision events are sent back to the smartphone to trigger vibration and audio, closing the loop.
...
- Android device: Any recent Android smartphone (tested on Samsung Galaxy series)
...
Build a Release APK in Android Studio, and install it on your smartphone.
...
Ensure both smartphone and PC are on the same Wi-Fi.
...
#### 5. Run and test
Launch the app, select Left/Right hand and Y/Z axis. Perform pinch/drag/rotation and check that Unity hands move. On collisions, the smartphone should vibrate.


## Results
Using two smartphones, MobileBiHap enables independent control of left and right virtual hands. Each hand follows its configured axis mode (Y or Z) for translation and rotation. The system supports grasping objects, lifting them, rotating while held, and then releasing to drop, all driven by real-time touch input from the smartphones. The system demonstrates stable real-time performance with low perceptual latency, enabling natural bi-manual manipulation without dedicated haptic hardware.
